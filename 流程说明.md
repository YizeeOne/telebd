...
- 派生指标（基于原始字段组合）：
  - 空间不均匀：`delta_rsrp`、`delta_cov2` 及其绝对值
  - 有效吞吐：`effective_dl`、`effective_ul`
  - 尾部拖累：`dl_tail_drag`、`ul_tail_drag`
  - 掉线强度：`dl/ul_drops_per_hour`、`dl/ul_drops_per_km`
  - RedCap 可用性：`availability`、`stay_eff`
  - 移动惩罚：`mobility_penalty`（DT 相对 CQT）
- 综合评分（加权）：覆盖/体验/稳定/空间不均匀/波动五类分项加权
- 输出聚合表：
  ...
  - `output/*_operator_composite_weighted.csv`
  - `output/*_significance.csv`
  - `output/*_mobility_penalty.csv`
  - `output/*_mobility_penalty_detail.csv`
  - `output/*_mobility_penalty_rank.csv`
...
- 一级场景对比柱状图
  ...
  - 维度：一级场景为 X，运营商为系列
- 派生指标图表（城市/一级场景/趋势）
  - 目录：`output/figures/derived/`
  - 文件：`*_derived_*_city_operator_bar.png`、`*_derived_*_scene1_operator_bar.png`、`*_derived_*_scene_trend.png`
- 运营商雷达图（归一化）
...
总结中包含缺失率、加权对比、稳健均值、综合评分（等权+加权）、派生指标与显著性检验提示。

## 6. 产出文件清单
- 脚本：`analysis.py`
- 清洗数据：`output/cleaned_*.csv`
- 聚合表：`output/*_mean.csv`、`output/*_scene1_operator_mean.csv`
- 加权/稳健/综合/显著性：`output/*_operator_weighted.csv`、`output/*_operator_robust.csv`、`output/*_operator_composite.csv`、`output/*_operator_composite_weighted.csv`、`output/*_significance.csv`
- 移动惩罚：`output/*_mobility_penalty.csv`、`output/*_mobility_penalty_detail.csv`、`output/*_mobility_penalty_rank.csv`
