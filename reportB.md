...
含义：该图展示剔除除夕后的周一至周日日均流量相对均值偏移（含周末，为当日/周均-1）。
...
含义：该图展示未剔除除夕的周一至周日日均用户数相对均值偏移（含周末，为当日/周均-1）。
