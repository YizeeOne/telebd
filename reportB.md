...
含义：该图展示除夕夜20:00的地理气泡分布（大小=流量，颜色=场景）。
意义：除夕夜气泡图, 同时呈现规模与场景类型差异。
用法：除夕夜气泡图, 辅助识别节假日高负载场景区域。

![工作日20:00气泡图（3月）](report_assets/section4/fig11_bubble_weekday_20.png)
含义：该图展示3月工作日20:00的地理气泡分布（大小=流量，颜色=场景）。
意义：工作日气泡图, 作为常态空间分布对照。
用法：工作日气泡图, 定位节假日与工作日的结构性差异。
